...
class VideoBox(QTabWidget): #封装类
    ...
    def tab1UI(self):   #设置
        ...
        self.tableWidget = QTableWidget()  #显示数据表格的控件
    ...
    def choose_pic(self):  #选择图片函数
        self.pic_choose_Button.setEnabled(True)    #激活按钮
        self.imagename, _ = QFileDialog.getOpenFileName(self, 'Choose photo', './images/', "Image files (*.jpg *.gif)")
                                                        #加载图片（文本框标题，打开路径，过滤文件）
        self.pictureLabel.setPixmap(QPixmap(self.imagename))    #图片缩放
        self.predict(self.imagename)    #预测图片


    def predict(self,image_path):  #预测函数预处理图片
        rgb_image = cv2.imread(image_path)  #图片读取
        gray_image = load_image(image_path, grayscale=True) #把图片转换为灰色图像
        gray_image = np.squeeze(gray_image) #压缩图片（去掉矩阵里维度为1的维度）
        gray_image = gray_image.astype('uint8') #数据类型转换
    ...
    def crop_pic(self):  #拍照
        self.camera_crop_Button.setEnabled(True)    #激活按钮
        ...
        while True:
            if self.crop_num%3==1:
                ...
                if cv2.waitKey(1) & 0xff == ord('A'):#防止bug(按A拍照）
                    break
